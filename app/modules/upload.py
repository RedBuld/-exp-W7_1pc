...
from ..forms import UploadForm, UploadIconForm
from ..tools import check_rank_user, check_rank, get_filename, remove_img, remove_icon
...
@upload_module.route('/icon', methods=['GET', 'POST'])
@login_required
def icon():
    if not check_rank(3):
        return abort(403)
    ...
    if request.method == 'POST':
        ...
        for i in u:
            ...
            db.session.delete(i)
        if form.validate_img():
            filename = get_filename(app.config['UPLOADS_ICON_FOLDER'], form.img.data.filename)
            form.img.data.save(app.config['UPLOADS_ICON_FOLDER'] + '/' + filename)
            u2 = Upload()
            u2.init(filename)
            db.session.add(u2)
            db.session.commit()
            return redirect(url_for('upload_module.icon', path=filename, absolute_path='/uploads/icons/'+filename))
        db.session.commit()
    return render_template('upload/icon.html', form=form, filename=request.args.get('path'))